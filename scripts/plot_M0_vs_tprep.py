...
for roi, color in zip(rois, palette):
    ...
    # tprep = data[:, 11]
    # M0 = data[:,3]/(data[:, 9] + data[:, 3]) 
    # error_M0 = M0*np.sqrt((data[:, 10]/data[:, 9])**2 + ( (data[:, 4] + data[:,10]) / (data[:, 3] + data[:,9]) )**2)

    # Obtener los índices que ordenarían grad
    sorted_indices = np.argsort(tprep)
    # Ordenar grad y M0 usando esos índices
    tprep = tprep[sorted_indices]
    ...
    ax2.errorbar(tprep, M0, yerr=error_M0,  fmt='o-', markersize=3, linewidth=2, capsize=5)
    #ax2.plot(tprep, M0, 'o-', markersize=7, linewidth=2, color = color)
    ax2.set_xlabel("Tiempo de preparación [días]", fontsize=27)
...
fig2.tight_layout()
fig2.savefig(f"{directory}/{roi}_M0_{zone}_vs_tprep.png", dpi=600)
fig2.savefig(f"{directory}/{roi}_M0_{zone}_vs_tprep.pdf")