...
for roi, color in zip(rois, palette):
    ...
    x = np.delete(x, [2,11,12,13])
    f = np.delete(f, [2,11,12,13])

    #modelo M_nogse_rest_dist
    model = lmfit.Model(nogse.M_nogse_restdist, independent_vars=["TE", "G", "N", "x", "D0"], param_names=["l_c_mode", "sigma", "M0"])
    model.set_param_hint("M0", value=500, max = 20000)
    model.set_param_hint("l_c_mode", value=6.0, min = 1.0, max = 20.0)
    model.set_param_hint("sigma", value=0.01, min = 0.001, max=1.0)
    ...
    fit = nogse.M_nogse_restdist(float(tnogse), float(g), n, x_fit, l_c_fit, sigma_fit, M0_fit, D0)
