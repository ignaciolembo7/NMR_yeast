...
for roi, g, color in zip(rois, gs, palette):
    ...
    ax1.plot(tnogse, l_c, 'o-', markersize=7, linewidth=2, color = color, label=f"{g}")
    ...
    ax2.plot(tnogse,l_c, 'o-', markersize=7, linewidth=2, color = color, label=f"{g}")
...
fig2.tight_layout()
fig2.savefig(f"{directory}/lc_vs_tnogse_allG.png", dpi=600)
fig2.savefig(f"{directory}/lc_vs_tnogse_allG.pdf")