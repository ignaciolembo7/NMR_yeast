...
folder = "fit_contrast_vs_g_mixto_mixtodist"
A0 = "con_A0"
D0_ext = 2.3e-12 # extra
D0_int = 0.65e-12 # 0.7e-12 # intra
exp = 1 #int(input('exp: '))
slic = 0 # slice que quiero ver
modelo = "Mixto+MixtoDist" 
...
for tnogse in tnogses: 
    ...
    for roi, color in zip(rois,palette):
        ...
        model = lmfit.Model(nogse.fit_contrast_vs_g_mixto_mixtodist, independent_vars=["TE", "G", "N", "D0_1", "D0_2"], param_names=["tc_1","alpha_1","M0_1","lc_mode_2","alpha_2","sigma_2","M0_2"])
        model.set_param_hint("M0_1", value=0.55, min=0, max = 1.0, vary = 1)
        model.set_param_hint("M0_2", expr="1 - M0_1")
        #model.set_param_hint("M0_2", value=0.45, min=0, max = 1.0, vary = 0)
        model.set_param_hint("tc_1", value=3.0, min = 0.1, max = 15.0, vary = 1)
        model.set_param_hint("lc_mode_2", value=1.7, min = 0.5, max = 10.0, vary = 1)
        model.set_param_hint("sigma_2", value=0.23, min = 0, max=2.0, vary = 1)
        model.set_param_hint("alpha_1", value=0.3, min = 0, max=1, vary = 1)
        model.set_param_hint("alpha_2", value=0.1, min = 0, max=1, vary = 1)
        ...
        fit = nogse.fit_contrast_vs_g_mixto_mixtodist(tnogse, g_fit, n, tc1_fit, alpha1_fit, M01_fit, D0_ext, lc2_fit, sigma2_fit, alpha2_fit, M02_fit, D0_int)
        ...
        with open(f"../results_{file_name}/{folder}/{roi}_parameters_vs_tnogse.txt", "a") as a:
            print(tnogse, tc1_fit, tc1_error, alpha1_fit, alpha1_error, M01_fit, M01_error, lc2_fit, lc2_error, sigma2_fit, sigma2_error, alpha2_fit, alpha2_error, M02_fit, M02_error, file=a)

        fig2, ax2 = plt.subplots(figsize=(8,6)) 

        lc2_median = lc2_fit*np.exp(sigma2_fit**2)
        lc2_mid = lc2_median*np.exp((sigma2_fit**2)/2)
        lc = np.linspace(0.01, 15, 1000) #asi esta igual que en nogse.py
        ...
        with open(f"{directory}/parameters_tnogse={tnogse}_N={int(n)}.txt", "a") as a:
            print(roi,  " - lc_mode_1 = ", tc1_fit, "+-", tc1_error, file=a)
