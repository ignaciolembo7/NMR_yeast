...
folder = "simulation_nogse_vs_x_restdist"
...
directory = f"../results_{file_name}/{folder}/slice={slic}/tnogse={tnogse}_N={int(n)}_lcmode={lc_mode}_sigma={sigma}_D0={D0_int}"
...
for g,color in zip(gs,palette):
    ...
    f = nogse.fit_nogse_vs_x_restdistmode(tnogse, g, n, x, lc_mode, sigma, M0, D0_int)
    error = 0
...
fig.tight_layout()
fig.savefig(f"{directory}/nogse_vs_x_tnogse={tnogse}_N={int(n)}_lcmode={lc_mode}_sigma={sigma}_D0={D0_int}.pdf")
fig.savefig(f"{directory}/nogse_vs_x_tnogse={tnogse}_N={int(n)}_lcmode={lc_mode}_sigma={sigma}_D0={D0_int}.png", dpi=600)
plt.close(fig)