...
folder = "globalfit_M0_lc_sigma_alpha_nogse_vs_x_mixto_restdist"
A0 = "sin_A0"
...
params.add(f'tc1', value=2.0, min=0.1, max=20.0, vary = 0)
params.add(f'alpha1', value=0.57, min=0.0, max=1.0, vary = 0)
params.add(f'lc2', value=1.71, min=0.1, max=4.0, vary = 0) 
params.add('sigma2', value=0.23, min=0.1, max=1.0, vary=0)
params.add(f'alpha2', value=0.0, min=0.0, max=1.0, vary = 0)
params.add('M01', value=5000, min=1, max=5000, vary=1)
#params.add("M0_2", expr="(M0_1 + M0_2)*0.45", min=1, max = 5000)
params.add('M02', value=100, min=1, max=5000, vary= 1)
...
def objective_function(params, x_list=xs, fs_list=fs):
    ...
    for i, (x, fs_data) in enumerate(zip(x_list, fs_list)):
        ...
        model = nogse.fit_nogse_vs_x_mixto_mixtodist(tnogses[i], gs[i], n, x, tc1_fit, alpha1_fit, M01_fit, D0_ext, lc2_fit, sigma2_fit, alpha2_fit, M02_fit, D0_int)
...
for i in range(len(xs)):
    ...
    fit = nogse.fit_nogse_vs_x_mixto_mixtodist(tnogses[i], gs[i], n, x_fit, tc1_fit, alpha1_fit, M01_fit, D0_ext, lc2_fit, sigma2_fit, alpha2_fit, M02_fit, D0_int)
...
with open(f"../results_{file_name}/{folder}/{rois[0]}_parameters_vs_tnogse.txt", "a") as a:
    print(tnogses[0], tc1_fit, tc1_error, alpha1_fit, alpha1_error, M01_fit, M01_error, lc2_fit, lc2_error, lc2_median, 0, lc2_mid, 0, sigma2_fit, sigma2_error, alpha2_fit, alpha2_error, M02_fit, M02_error, file=a) 
