...
for i in ["G2","G3"]: 
    ...
    data = np.loadtxt(f"{directory}/tauc_vs_tnogse_" + i + ".txt")
    ...
    ax1.errorbar(tnogse, t_c, fmt='o-', markersize=3, linewidth=2, capsize=5, label=f"{i}") #  yerr=error_t_c,
    #ax.plot(tnogse, tau_c, 'o-', markersize=3, linewidth=2)
    ax1.set_xlabel("Tiempo de difusión $\mathrm{NOGSE}$ [ms]", fontsize=18)
    ax1.set_ylabel("Tiempo de correlación $\\tau_c$ [ms]", fontsize=18)
    ...
    fig1.tight_layout()
    fig1.savefig(f"{directory}/tauc_vs_tnogse_" + i + ".png", dpi=600)
    fig1.savefig(f"{directory}/tauc_vs_tnogse_" + i + ".pdf")

    ax2.errorbar(tnogse, t_c,  fmt='o-', markersize=3, linewidth=2, capsize=5, label=f"{i}") #yerr=error_t_c,
    #ax.plot(tnogse, tau_c, 'o-', markersize=3, linewidth=2)
...
fig2.tight_layout()
fig2.savefig(f"{directory}/tauc_vs_tnogse_allG.png", dpi=600)
fig2.savefig(f"{directory}/tauc_vs_tnogse_allG.pdf")