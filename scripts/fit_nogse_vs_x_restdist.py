...
for tnogse, g in zip(tnogses, G4):
    ...
    folder = "fit_nogse_vs_x_restdist"
    exp = 1 #int(input('exp: '))
    ...
    for roi, color in zip(rois, palette):
        ...
        model = lmfit.Model(nogse.fit_nogse_vs_x_restdistmode, independent_vars=["TE", "G", "N", "x"], param_names=["lc_mode", "sigma", "M0", "D0"])
        model.set_param_hint("M0", value=1, max = 10000, vary=1)
        model.set_param_hint("lc_mode", value=2.14, min = 1.0, max = 10.0, vary=1)
        model.set_param_hint("sigma", value=0.3, min = 0.1, max=1.0, vary=1)
        model.set_param_hint("D0", value=1.0e-12, min = 0.0, max = D0_ext, vary=0)
        ...
        fit = nogse.fit_nogse_vs_x_restdistmode(float(tnogse), float(g), n, x_fit, lc_fit, sigma_fit, M0_fit, D0_fit)
        ...
        l_c = np.linspace(0.01, 20, 1000) #asi esta igual que en nogse.py
        ...
        with open(f"../results_{file_name}/{folder}/{roi}_parameters_vs_tnogse_g={num_grad}.txt", "a") as a:
            print(tnogse, g, lc_fit, lc_error, sigma_fit, sigma_error, M0_fit, M0_error, D0_fit, D0_error, file=a)
