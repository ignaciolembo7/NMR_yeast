...
folder = "fit_contrast_vs_g_free_rest"
A0 = "con_A0"
...
for tnogse in tnogses: 
    ...
    for roi, color in zip(rois,palette):
        ...
        model.set_param_hint("M0_1", value=0.47, min=0, max = 1.0, vary = 1) #extracelular
        model.set_param_hint("M0_2", expr="1 - M0_1") 
        #model.set_param_hint("M0_2", value=0.15, min=0, max = 1.0, vary = 1) #intracelular
        model.set_param_hint("tc_2", value=2.21, min = 0, max = 10.0, vary = 1) #intracelular
        model.set_param_hint("alpha_1", value=0.57, min = 0, max=1, vary = 1) #extracelular
    ...
    fig.tight_layout()
    fig.savefig(f"../results_{file_name}/{folder}/{A0}/contrast_vs_g_tnogse={tnogse}_N={int(n)}_exp={exp}.pdf")
