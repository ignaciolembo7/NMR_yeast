...
folder = "fit_contrast_vs_g_free_mixto"
A0 = "con_A0"
...
for roi, color in zip(rois,palette):
    ...
    model.set_param_hint("M0_1", value=0.65, min=0, max = 10000.0, vary = 0) #extracelular
    #model.set_param_hint("M0_2", expr="1 - M0_1") 
    model.set_param_hint("M0_2", value=0.3, min=0, max = 10000.0, vary = 0) #intracelular
    model.set_param_hint("tc_2", value=1.74, min = 0, max = 10.0, vary = 0) #intracelular
    model.set_param_hint("alpha_1", value=0.58, min = 0, max=1, vary = 0) #extracelular
    model.set_param_hint("alpha_2", value=0.0079, min = 0, max=1, vary = 0) #intracelular
    ...
    fit = nogse.fit_contrast_vs_g_free_mixto(tnogse, g_fit, n, alpha_1_fit, M01_fit, D0_ext, tc_2_fit, alpha_2_fit, M02_fit, D0_int)
    fit_1 = nogse.fit_contrast_vs_g_free(tnogse, g_fit, n, alpha_1_fit, M01_fit, D0_ext)
    ...
    with open(f"{directory}/parameters_tnogse={tnogse}_N={int(n)}.txt", "a") as a:
        print("roi",  " - alpha_1 = ", alpha_1_fit, "+-", error_alpha_1, file=a)
    ...
    with open(f"../results_{file_name}/{folder}/{A0}/{roi}_parameters_vs_tnogse.txt", "a") as a:
        print(tnogse, alpha_1_fit, error_alpha_1, M01_fit, error_M01, tc_2_fit, error_tc_2, alpha_2_fit, error_alpha_2, M02_fit, error_M02, file=a)
