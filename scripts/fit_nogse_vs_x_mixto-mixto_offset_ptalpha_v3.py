...
parameters = {(tnogse, num_grad): {'tc1': [], 'tc1_error': [], 'alpha_1': [], 'C': [], 'C_error': [], 'chi': []} for tnogse in tnogses for num_grad in num_grads}
...
for tnogse, gs in zip(tnogses, gss):
    ...
    for num_grad, g, alphas1, color in zip(num_grads, gs, alphass1, palette):
        ...
        folder = f"fit_nogse_vs_x_mixto-rest_pt_alpha_M01=0.70_D02={D0_int}"
        ...
        for i, alpha1 in enumerate(alphas1):
            #fit_nogse_vs_x_mixto_mixto_offset(TE, G, N, x, tc_1, alpha_1, M0_1, D0_1, tc_2, alpha_2, M0_2, D0_2, C)
            ...
            model.set_param_hint("M0_1", expr="(0.70/0.30)*M0_2")
            ...
            print(f"Ajuste {i + 1} para alpha = {alpha1}")
            ...
            parameters[(tnogse, num_grad)]['tc1'].append(tc1_fit)
...
# Graficar alpha vs t_c para todos los num_grad para cada tnogse
for tnogse in tnogses:
    fig2, ax2 = plt.subplots(figsize=(8, 6))
    for num_grad, color in zip(num_grads, palette):
        tc_values = parameters[(tnogse, num_grad)]['tc1']
        alpha_values = parameters[(tnogse, num_grad)]['alpha_1']
        ax2.plot(alpha_values, tc_values, "-", linewidth = 2,  color=color, label=num_grad)
        ...
        ax2.scatter(alpha_values, tc_values, c=chi_values, cmap='plasma', s=100, edgecolor='black', linewidth=0.5)

    cbar = plt.colorbar(ax2.scatter(alpha_values, tc_values, c=chi_values, cmap='plasma', s=100, edgecolor='black', linewidth=0.5))
    ...
    fig2.tight_layout()
    fig2.savefig(f"../results_{file_name}/{folder}/{A0}/tc1_vs_alpha1_tnogse={tnogse}.pdf")
    fig2.savefig(f"../results_{file_name}/{folder}/{A0}/tc1_vs_alpha1_tnogse={tnogse}.png", dpi=600)
    plt.close(fig2)
...
for num_grad in num_grads:
    ...
    for tnogse, color in zip(tnogses, palette):
        tc_values = parameters[(tnogse, num_grad)]['tc1']
        alpha_values = parameters[(tnogse, num_grad)]['alpha_1']
        ax3.plot(alpha_values, tc_values, "-", linewidth = 2, color=color, label=tnogse)
        #color del punto asociado al valor del chi cuadradado para ese ajuste con barra de de color al costado de la figura
        chi_values = parameters[(tnogse, num_grad)]['chi']
        ax3.scatter(alpha_values, tc_values, c=chi_values, cmap='plasma', s=100, edgecolor='black', linewidth=0.5)
        
    cbar = plt.colorbar(ax3.scatter(alpha_values, tc_values, c=chi_values, cmap='plasma', s=100, edgecolor='black', linewidth=0.5))
    cbar.set_label('Chi cuadrado', fontsize=15)
    cbar.ax.tick_params(labelsize=15)
    
    ax3.legend(title="$T_\\mathrm{NOGSE}$ [ms]", title_fontsize=15, fontsize=15, loc='best')
    ax3.set_ylabel("Tiempo de correlación $\\tau_c$ [ms]", fontsize=27)
    ...
    fig3.tight_layout()
    fig3.savefig(f"../results_{file_name}/{folder}/{A0}/tc1_vs_alpha1_G={num_grad}.pdf")
    fig3.savefig(f"../results_{file_name}/{folder}/{A0}/tc1_vs_alpha1_G={num_grad}.png", dpi=600)
    plt.close(fig3)
