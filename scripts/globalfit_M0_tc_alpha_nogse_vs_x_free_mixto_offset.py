...
folder = "globalfit_M0_tc_alpha_nogse_vs_x_free_rest_offset"
A0 = "sin_A0"
...
params.add(f'tc2', value=2.21, min=1.0, max=15.0, vary = 1)
...
def objective_function(params, x_list=xs, fs_list=fs):
    ...
    for i, (x, fs_data) in enumerate(zip(x_list, fs_list)):
        ...
        C = params['C'].value
        model = nogse.fit_nogse_vs_x_free_mixto_offset(tnogses[i], gs[i], n, x, alpha1_fit, M01_fit, D0_ext, tc2_fit, alpha2_fit, M02_fit, D0_int, C)
...
for i in range(len(xs)):
    ...
    fit = nogse.fit_nogse_vs_x_free_mixto_offset(tnogses[i], gs[i], n, x_fit, alpha1_fit, M01_fit, D0_ext, tc2_fit, alpha2_fit, M02_fit, D0_int, C_fit)
...
with open(f"../results_{file_name}/{folder}/{rois[0]}_parameters_vs_tnogse.txt", "a") as a:
    print(tnogses[0], alpha1_fit, alpha1_error, M01_fit, M01_error, tc2_fit, tc2_error, alpha2_fit, alpha2_error, M02_fit, M02_error, C_fit, C_error, file=a) 
