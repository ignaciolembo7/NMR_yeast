...
folder = "globalfit_M0_alpha_nogse_vs_x_mixto_rest"
A0 = "sin_A0"
...
for i in range(len(xs)-1):
    ...
    params.add(f'tc_2_{i+1}', value=1.8, min=0.1, max=50.0, vary = 0)
    params.add(f'alpha_2_{i+1}', value=0.0, min=0.0, max=1.0, vary = 0)

params.add(f'tc_1_{4}', value=100000, min=0.1, max=100000.0, vary = 0)
params.add(f'tc_2_{4}', value=1.8, min=0.1, max=50.0, vary = 0)    
params.add(f'alpha_2_{4}', value=0.0, min=0.0, max=1.0, vary = 0)
params.add('M0_1', value=5000, min=0, max=10000, vary=1)
params.add('M0_2', value=5000, min=0, max=10000, vary=1)
#params.add('M0_2', expr="0.42857*M0_1", min=0, max=10000, vary=1)
params.add(f'alpha_1', value=alpha_ext, min=0.1, max=1.0, vary = 0)
...
def objective_function(params, x_list=xs, fs_list=fs):
    ...
    for i, (x, fs_data) in enumerate(zip(x_list, fs_list)):
        ...
        alpha_1_fit = params[f'alpha_1'].value
...
alpha_1_fit = result.params[f'alpha_1'].value
alpha_1_error = result.params[f'alpha_1'].stderr
...
for i in range(len(xs)):
    ...
    with open(f"{directory}/parameters_tnogse={tnogses[0]}_g={gs[i]}_N={int(n)}_G={num_grads[i]}.txt", "a") as a:
        ...
        print("     ",  " - tc_2 = ", tc_2_fit, "+-", tc_2_errors, file=a)
    ...
    with open(f"../results_{file_name}/{folder}/{roi}_parameters_vs_tnogse_G={num_grads[i]}.txt", "a") as a:
        print(tnogses[0], gs[i], tc_1_fit, tc_1_error, alpha_1_fit, alpha_1_error, alpha_2_fit, alpha_2_error, file=a)

    with open(f"../results_{file_name}/{folder}/{roi}_parameters_vs_g_tnogse={tnogses[0]}.txt", "a") as a:
        print(gs[i], tnogses[0], tc_1_fit, tc_1_error, alpha_1_fit, alpha_1_error, alpha_2_fit, alpha_2_error, file=a)
...
with open(f"../results_{file_name}/{folder}/{roi}_parameters_vs_tnogse.txt", "a") as a:
    print(tnogses[0], alpha_1_fit, alpha_1_error, M01_fit, M01_error, M02_fit, M02_error, file=a)
