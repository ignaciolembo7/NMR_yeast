...
def objective_function(params, x_list=xs, fs_list=fs):
    ...
    for i, (x, fs_data) in enumerate(zip(x_list, fs_list)):
        ...
        model = nogse.fit_nogse_vs_x_free_mixtodist(tnogses[i], gs[i], n, x, alpha1_fit, M01_fit, D0_ext, lc2_fit, sigma2_fit, alpha2_fit, M02_fit, D0_int)
...
with open(f"../results_{file_name}/{folder}/{rois[0]}_parameters_vs_tnogse.txt", "a") as a:
    print(tnogses[0], alpha1_fit, alpha1_error, M01_fit, M01_error, lc2_fit, lc2_error, lc2_median, 0, lc2_mid, 0, sigma2_fit, sigma2_error, alpha2_fit, alpha2_error, M02_fit, M02_error, file=a) 
