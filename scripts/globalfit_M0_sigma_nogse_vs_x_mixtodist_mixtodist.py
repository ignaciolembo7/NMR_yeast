...
folder = "globalfit_nogse_vs_x_mixtodist_mixtodist"
A0 = "sin_A0"
...
for i in range(len(xs)):
    params.add(f'lc1_{i+1}', value=np.sqrt(2*(D0_ext*1e12)*0.69), min=0.5, max=10.0, vary = 0)
    params.add(f'alpha1_{i+1}', value=0.537, min=0.0, max=1.0, vary = 0)
    params.add(f'lc2_{i+1}', value=np.sqrt(2*(D0_int*1e12)*1.6), min=0.5, max=15.0, vary = 0)
    params.add(f'alpha2_{i+1}', value=0.0, min=0.0, max=1.0, vary = 0)
params.add('M01', value=100, min=0, max=10000, vary=1)
params.add('M02', value=100, min=0, max=10000, vary=1)
params.add('sigma1', value=1, min=0.1, max=5.0, vary=1)
params.add('sigma2', value=1, min=0.1, max=2.0, vary=1)
...
def objective_function(params, x_list=xs, fs_list=fs):
    ...
    for i, (x, fs_data) in enumerate(zip(x_list, fs_list)):
        lc1_fit = params[f'lc1_{i+1}'].value
        alpha1_fit = params[f'alpha1_{i+1}'].value
        M01_fit = params[f'M01'].value
        sigma1_fit = params[f'sigma1'].value
        lc2_fit = params[f'lc2_{i+1}'].value
        alpha2_fit = params[f'alpha2_{i+1}'].value
...
error_M01 = result.params[f'M01'].stderr
M02_fit = result.params[f'M02'].value
error_M02 = result.params[f'M02'].stderr
...
for i in range(len(xs)):
    ...
    lc1_fit = result.params[f'lc1_{i+1}'].value
    lc1_error = result.params[f'lc1_{i+1}'].stderr
    alpha1_fit = result.params[f'alpha1_{i+1}'].value
    alpha1_error = result.params[f'alpha1_{i+1}'].stderr
    lc2_fit = result.params[f'lc2_{i+1}'].value
    lc2_error = result.params[f'lc2_{i+1}'].stderr
    alpha2_fit = result.params[f'alpha2_{i+1}'].value
    alpha2_error = result.params[f'alpha2_{i+1}'].stderr
