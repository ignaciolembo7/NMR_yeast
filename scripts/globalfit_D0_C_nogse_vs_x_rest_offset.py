...
folder = "globalfit_C_nogse_vs_x_rest_offset"
A0 = "sin_A0"
...
for i in range(len(xs)):
    ...
    params.add(f'tc{i+1}', value=1.75, min=0.1, max=5.0, vary = 1)
...
for i in range(len(xs)):
    ...
    tc_errors.append(result.params[f'tc{i+1}'].stderr)
...
with open(f"{directory}/parameters_N={int(n)}.txt", "a") as a:
    print("roi",  " - C = ", C_fit, " +/- ", C_error, file=a)
    ...
    for i in range(len(xs)):
        print(f"    - tc_{i+1} = {tc_fits[i]} +- {tc_errors[i]}", file=a)
...
for i in range(len(xs)):
    ...
    fit = nogse.M_nogse_rest_offset(float(tnogses[i]), float(gs[i]), n, x_fit, tc_fits[i], M0_fits[i], D0_fit, C_fit)
...
with open(f"../results_{file_name}/{folder}/{roi}_parameters_vs_tnogse.txt", "a") as a:
    print(tnogse, C_fit, C_error, file=a) 

with open(f"../results_{file_name}/{folder}/{roi}_M0_vs_tnogse.txt", "a") as a:
    for i in range(len(xs)):
        print(tnogses[i], M0_fits[i], M0_errors[i], file=a)
    
with open(f"../results_{file_name}/{folder}/{roi}_tc_vs_tnogse.txt", "a") as a:
    for i in range(len(xs)):
        print(tnogses[i], tc_fits[i], tc_errors[i], file=a)