...
alpha_vs_tc = {(tnogse): {'tc1': [], 'alpha': [], 'chi': []} for tnogse in tnogses}

file_name = "levaduras_20240613"
folder = "fit_contrast_vs_g_mixto_rest_pt_alpha"
A0 = "con_A0"
exp = 1
n = 2
slic = 0
...
for tnogse, alphas, color in zip(tnogses, alphass, palette):
        ...
        for i, alpha in enumerate(alphas):
            ...
            model.set_param_hint("M0_1", value=0.8, min=0, max = 1.0, vary = 0) #extracelular
            model.set_param_hint("M0_2", expr="1 - M0_1") 
            #model.set_param_hint("M0_2", value=0.15, min=0, max = 1.0, vary = 1) #intracelular
            model.set_param_hint("tc_1", value=3.0, min = 0.1, max = 30.0, vary = 1) #extracelular
            ...
            alpha_vs_tc[(tnogse)]['tc1'].append(tc1_fit)
            alpha_vs_tc[(tnogse)]['chi'].append(result.chisqr)

            g_fit = np.linspace(0, np.max(g), num=1000)
            fit = nogse.fit_contrast_vs_g_mixto_mixto(tnogse, g_fit, n, tc1_fit, alpha, M01_fit, D0_ext, tc2_fit, alpha2_fit, M02_fit, D0_int)
...
# Graficar alpha vs t_c para todos los tnogse para cada num_grad
fig3, ax3 = plt.subplots(figsize=(8, 6))
for tnogse, color in zip(tnogses, palette):
    tc_values = alpha_vs_tc[(tnogse)]['tc1']
    alpha_values = alpha_vs_tc[(tnogse)]['alpha']
    ...
    chi_values = alpha_vs_tc[(tnogse)]['chi']
...
ax3.legend(title="$T_\\mathrm{NOGSE}$", title_fontsize=15, fontsize=15, loc='best')
...
title = ax3.set_title(f"{modelo} | $N$ = {n} | slice = {slic}", fontsize=15)

fig3.tight_layout()
fig3.savefig(f"../results_{file_name}/{folder}/{A0}/tc1_vs_alpha.pdf")
fig3.savefig(f"../results_{file_name}/{folder}/{A0}/tc1_vs_alpha.png", dpi=600)
plt.close(fig3)