...
folder = "fit_contrast_vs_g_mixto_mixto_free"
A0 = "con_A0"
D0_ext = 2.3e-12 # extra
D0_int = 0.7e-12 # 0.7e-12 # intra
exp = 1 #int(input('exp: '))
slic = 0 # slice que quiero ver
modelo = "Mixto+Mixto+Free"
...
for roi, color in zip(rois,palette):
    ...
    model = lmfit.Model(nogse.fit_contrast_vs_g_mixto_mixto_free, independent_vars=["TE", "G", "N", "D0_2", "D0_3"], param_names=["tc_1", "alpha_1", "M0_1", "D0_1", "tc_2", "alpha_2", "M0_2", "alpha_3", "M0_3"])
    model.set_param_hint("M0_1", value=0.3, min=0, max = 1.0, vary = 1) #extracelular
    #model.set_param_hint("M0_2", expr="1 - M0_1") 
    model.set_param_hint("M0_2", value=0.3, min=0, max = 1.0, vary = 1) #intracelular
    model.set_param_hint("M0_3", value=0.8, min=0, max = 1.0, vary = 1) #bulk
    model.set_param_hint("tc_1", value=9.7, min = 0, max = 100.0, vary = 1) #extracelular
    model.set_param_hint("tc_2", value=2.0, min = 0, max = 10.0, vary = 1) #intracelular
    model.set_param_hint("alpha_1", value=0.0, min = 0, max=1, vary = 0) #extracelular
    model.set_param_hint("alpha_2", value=0.0, min = 0, max=1, vary = 0) #intracelular
    model.set_param_hint("alpha_3", value=1.0, min = 0, max=1, vary = 0) #bulk
    model.set_param_hint("D0_1", value=1.639e-12, min = 0, max=2.3e-12, vary = 0) #extracelular

    params = model.make_params()

    result = model.fit(f, params, TE=float(tnogse), G=g, N=n, D0_2=D0_int, D0_3=D0_ext) 
    ...
    D0_1_fit = result.params["D0_1"].value
    error_D0_1 = result.params["D0_1"].stderr 
    
    g_fit = np.linspace(np.min(g), np.max(g), num=1000)
    fit = nogse.fit_contrast_vs_g_mixto_mixto_free(tnogse, g_fit, n, tc_1_fit, alpha_1_fit, M01_fit, D0_1_fit, tc_2_fit, alpha_2_fit, M02_fit, D0_int, alpha_3_fit, M0_3_fit, D0_ext)
    fit_1 = nogse.fit_contrast_vs_g_mixto(tnogse, g_fit, n, tc_1_fit, alpha_1_fit, M01_fit, D0_1_fit)
    fit_2 = nogse.fit_contrast_vs_g_mixto(tnogse, g_fit, n, tc_2_fit, alpha_2_fit, M02_fit, D0_int)
    fit_3 = nogse.fit_contrast_vs_g_free(tnogse, g_fit, n, alpha_3_fit, M0_3_fit, D0_ext)
    ...
    with open(f"{directory}/parameters_tnogse={tnogse}_N={int(n)}.txt", "a") as a:
        ...
        print("     ",  " - D0_1 = ", D0_1_fit, "+-", error_D0_1, file=a)
