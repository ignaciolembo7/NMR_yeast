...
folder = "globalfit_M0_tc_alpha_nogse_vs_x_mixto_mixto"
A0 = "sin_A0"
...
params.add(f'tc1', value= 0.1, min=0.1, max=10.0, vary = 1)
params.add(f'alpha1', value=0.5, min=0.1, max=1.0, vary = 1)
params.add(f'tc2', value=1.95, min=1.0, max=15.0, vary = 1)
...
def objective_function(params, x_list=xs, fs_list=fs):
    ...
    for i, (x, fs_data) in enumerate(zip(x_list, fs_list)):
        ...
        model = nogse.fit_nogse_vs_x_mixto_mixto(tnogses[i], gs[i], n, x, tc1_fit, alpha1_fit, M01_fit, D0_ext, tc2_fit, alpha2_fit, M02_fit, D0_int)
...
for i in range(len(xs)):
    ...
    fit = nogse.fit_nogse_vs_x_mixto_mixto(tnogses[i], gs[i], n, x_fit, tc1_fit, alpha1_fit, M01_fit, D0_ext, tc2_fit, alpha2_fit, M02_fit, D0_int)
