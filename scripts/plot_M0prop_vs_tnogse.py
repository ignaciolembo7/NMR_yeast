...
for roi, color in zip(rois, palette):
    ...
    title = ax2.set_title(f"$M_0$ prop {zone} | $N$ = {n} | slice = {slic} ", fontsize=18)

fig2.tight_layout()
fig2.savefig(f"{directory}/{roi}_M0_prop_{zone}_vs_tnogse.png", dpi=600)
fig2.savefig(f"{directory}/{roi}_M0_prop_{zone}_vs_tnogse.pdf")