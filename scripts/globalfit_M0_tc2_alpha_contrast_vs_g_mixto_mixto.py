...
for i in range(len(gs)):
    params.add(f'tc1_{i+1}', value=2.2, min=0.1, max=15.0, vary = 1)

params.add(f'alpha1', value=0.4, min=0.1, max=1.0, vary = 1)
params.add(f'tc2', value=2.2, min=0.1, max=15.0, vary = 1)
params.add(f'alpha2', value=0.0, min=0.0, max=1.0, vary = 0)
params.add('M01', value=0.55, min=0, max=1, vary=1)
...
def objective_function(params, x_list=gs, fs_list=fs):
    ...
    for i, (g, fs_data) in enumerate(zip(x_list, fs_list)):
        tc1_fit = params[f'tc1_{i+1}'].value
        ...
        model = nogse.fit_contrast_vs_g_mixto_mixto(tnogses[i], gs[i], n, tc1_fit, alpha1_fit, M01_fit, D0_ext, tc2_fit, alpha2_fit, M02_fit, D0_int)
...
for i in range(len(gs)):
    tc1_fits.append(result.params[f'tc1_{i+1}'].value)
    tc1_errors.append(result.params[f'tc1_{i+1}'].stderr)
...
for tnogse, tc1_fit, color in zip(tnogses, tc1_fits, palette):
...
with open(f"../results_{file_name}/{folder}/{A0}/{roi}_tc1_vs_tnogse.txt", "a") as a:
    for i in range(len(gs)):
        print(tnogses[i], tc1_fits[i], tc1_errors[i], file=a)
