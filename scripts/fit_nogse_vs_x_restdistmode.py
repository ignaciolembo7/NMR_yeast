...
folder = "nogse_vs_x_fit_restdistmode_restdistmode_to_restdistmode"
A0 = "sin_A0"
...
directory = f"../results_{file_name}/{folder}/{D0_folder}/tnogse={tnogse}_g={g}_N={int(n)}_exp={exp}"
...
for roi, color in zip(rois, palette):
    ...
    data = np.loadtxt(f"../results_{file_name}/nogse_vs_x_data_restdistmode_restdistmode/slice={slic}/tnogse={tnogse}_g={g}_N={int(n)}_exp={exp}/{roi}_data_nogse_vs_x_tnogse={tnogse}_g={g}_N={int(n)}.txt")
    ...
    model = lmfit.Model(nogse.M_nogse_restdistmode, independent_vars=["TE", "G", "N", "x", "D0"], param_names=["l_c_mode", "sigma", "M0"])
    model.set_param_hint("M0", value=100, max = 1000000)
    model.set_param_hint("l_c_mode", value=6.0, min = 1.0, max = 20.0)
    model.set_param_hint("sigma", value=0.01, min = 0.001, max=2.5)
    ...
    fit = nogse.M_nogse_restdistmode(float(tnogse), float(g), n, x_fit, l_c_fit, sigma_fit, M0_fit, D0)
