...
for roi, tnogse in zip(rois, tnogses): 
    ...
    #ax1.errorbar(grad, l_c, fmt='o-', markersize=3, linewidth=2, capsize=5, label=f"{tnogse}") #  yerr=error_t_c,
    ax1.plot(grad, l_c, 'o-', markersize=7, linewidth=2, label=f"{tnogse}")
    ...
    fig1.tight_layout()
    fig1.savefig(f"{directory}/{roi}_lc_vs_g_tnogse={tnogse}.png", dpi=600)
    fig1.savefig(f"{directory}/{roi}_lc_vs_g_tnogse={tnogse}.pdf")

    #ax2.errorbar(grad, t_c,  fmt='o-', markersize=3, linewidth=2, capsize=5, label=f"{tnogse}") #yerr=error_t_c,
    ax2.plot(grad, l_c, 'o-', markersize=7, linewidth=2, label=f"{tnogse}")
...
fig2.tight_layout()
fig2.savefig(f"{directory}/lc_vs_g_alltnogse.png", dpi=600)
fig2.savefig(f"{directory}/lc_vs_g_alltnogse.pdf")