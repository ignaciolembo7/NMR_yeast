...
for i in range(len(xs)):
    ...
    params.add(f'C{i+1}', value=100, min = 0, vary=1)
...
for i in range(len(xs)):
    ...
    C_fits.append(result.params[f'C{i+1}'].value)
    C_errors.append(result.params[f'C{i+1}'].stderr)
...
with open(f"{directory}/parameters_N={int(n)}.txt", "a") as a:
    ...
    for i in range(len(xs)):
        print(f"    - C_{i+1} = {C_fits[i]} +- {C_errors[i]}", file=a)
...
for i in range(len(xs)):
    ...
    fit = nogse.M_nogse_rest_offset(float(tnogses[i]), float(gs[i]), n, x_fit, tc_fit, M0_fits[i], D0_fit, C_fits[i])
...
with open(f"../results_{file_name}/{folder}/{roi}_C_vs_tnogse.txt", "a") as a:
    for i in range(len(xs)):
        print(tnogses[i], C_fits[i], C_errors[i], file=a)