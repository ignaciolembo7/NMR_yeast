...
for roi, g, color in zip(rois, gs, palette):
    ...
    #promedio sin los valores en la posicion 5
    sigma_promedio = np.mean(sigma)
    sigma_promedio_error = np.std(sigma)
    ...
    #ax2.errorbar(tnogse, sigma, yerr=error_sigma, fmt='o-', markersize=3, linewidth=2, capsize=5, label=f"{g}") #yerr=error_t_c,
    ax2.plot(tnogse, sigma, 'o-', markersize=7, linewidth=2,  color = color)
    ax2.axhline(y=sigma_promedio, color='r', linestyle='--', label=f"Promedio = ({sigma_promedio:.2f} $\pm$ {sigma_promedio_error:.2f}) $\mu$m") 
...
fig2.tight_layout()
fig2.savefig(f"{directory}/sigma_{zone}_vs_tnogse.png", dpi=600)
fig2.savefig(f"{directory}/sigma_{zone}_vs_tnogse.pdf")