...
for tnogse, g in zip(tnogses, G4):
    ...
    folder = "fit_nogse_vs_x_mixtodist"
    exp = 1 #int(input('exp: '))
    ...
    for roi, color in zip(rois, palette):
        ...
        model = lmfit.Model(nogse.fit_nogse_vs_x_mixtodistmode, independent_vars=["TE", "G", "N", "x", "D0"], param_names=["lc_mode", "sigma", "alpha", "M0"])
        model.set_param_hint("M0", value=100, max = 10000, vary=1)
        model.set_param_hint("lc_mode", value=1.6, min = 0.1, max = 3.0, vary=1)
        model.set_param_hint("sigma", value=0.1, min = 0.01, max=2.5, vary=1)
        model.set_param_hint("alpha", value=0, min = 0.0, max=1.0, vary=1)
        ...
        fit = nogse.fit_nogse_vs_x_mixtodistmode(float(tnogse), float(g), n, x_fit, lc_fit, sigma_fit, alpha_fit, M0_fit, D0)
