...
for roi, tnogse in zip(rois, tnogses): 
    ...
    # Ordenar grad y l_c usando esos índices
    ...
    #ax1.errorbar(grad, t_c, fmt='o-', markersize=3, linewidth=2, capsize=5, label=f"{tnogse}") #  yerr=error_t_c,
    ax1.plot(grad, t_c, 'o-', markersize=7, linewidth=2, label=f"{tnogse}")
    ...
    fig1.tight_layout()
    fig1.savefig(f"{directory}/{roi}_tauc_vs_g_tnogse={tnogse}.png", dpi=600)
    fig1.savefig(f"{directory}/{roi}_tauc_vs_g_tnogse={tnogse}.pdf")

    #ax2.errorbar(grad, t_c,  fmt='o-', markersize=3, linewidth=2, capsize=5, label=f"{tnogse}") #yerr=error_t_c,
    ax2.plot(grad, t_c, 'o-', markersize=7, linewidth=2, label=f"{tnogse}")
...
fig2.tight_layout()
fig2.savefig(f"{directory}/tauc_vs_g_alltnogse.png", dpi=600)
fig2.savefig(f"{directory}/tauc_vs_g_alltnogse.pdf")