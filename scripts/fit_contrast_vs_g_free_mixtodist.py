...
folder = "fit_contrast_vs_g_free_mixtodist"
A0 = "con_A0"
...
for tnogse in tnogses: 
    ...
    for roi, color in zip(rois,palette):
        ...
        model = lmfit.Model(nogse.fit_contrast_vs_g_free_mixtodist, independent_vars=["TE", "G", "N", "D0_1", "D0_2"], param_names=["alpha_1","M0_1","lc_mode_2","sigma_2","alpha_2","M0_2"])
        model.set_param_hint("M0_1", value=0.56, min=0, max = 1.0, vary = 1)
        model.set_param_hint("M0_2", expr="1 - M0_1")
        #model.set_param_hint("M0_2", value=0.44, min=0, max = 1.0, vary = 1)
        model.set_param_hint("lc_mode_2", value=1.4, min = 0.5, max = 10.0, vary = 0)
        model.set_param_hint("sigma_2", value=0.36, min = 0.01, max=2.0, vary = 0)
        model.set_param_hint("alpha_1", value=0.57, min = 0, max=1, vary = 1)
        model.set_param_hint("alpha_2", value=0.001, min = 0.0, max=1, vary = 1)
        ...
        fit = nogse.fit_contrast_vs_g_free_mixtodist(tnogse, g_fit, n, alpha1_fit, M01_fit, D0_ext, lc2_fit, sigma2_fit, alpha2_fit, M02_fit, D0_int)
        ...
        fig1.tight_layout()
        fig1.savefig(f"../results_{file_name}/{folder}/{roi}_contrast_vs_g_tnogse={tnogse}_N={int(n)}_exp={exp}.pdf")
        fig1.savefig(f"../results_{file_name}/{folder}/{roi}_contrast_vs_g_tnogse={tnogse}_N={int(n)}_exp={exp}.png", dpi=600)
        plt.close(fig1)

        with open(f"../results_{file_name}/{folder}/{roi}_parameters_vs_tnogse.txt", "a") as a:
            print(tnogse, alpha1_fit, alpha1_error, M01_fit, M01_error, lc2_fit, lc2_error, sigma2_fit, sigma2_error, alpha2_fit, alpha2_error, M02_fit, M02_error, file=a)
        ...
        fig2.tight_layout()
        fig2.savefig(f"../results_{file_name}/{folder}/{roi}_dist_tnogse={tnogse}_N={int(n)}_exp={exp}.pdf")
        fig2.savefig(f"../results_{file_name}/{folder}/{roi}_dist_tnogse={tnogse}_N={int(n)}_exp={exp}.png", dpi=600)
        plt.close(fig2)

    fig.tight_layout()
    fig.savefig(f"../results_{file_name}/{folder}/contrast_vs_g_tnogse={tnogse}_N={int(n)}_exp={exp}.pdf")
    fig.savefig(f"../results_{file_name}/{folder}/contrast_vs_g_tnogse={tnogse}_N={int(n)}_exp={exp}.png", dpi=600)
    plt.close(fig)