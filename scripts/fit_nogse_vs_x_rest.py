...
for tnogse, g in zip(tnogses, G4):
    ...
    for roi, color in zip(rois,palette):
        ...
        data = np.loadtxt(f"../results_{file_name}/nogse_vs_x_data/slice={slic}/{A0_folder}/tnogse={tnogse}_g={g}_N={int(n)}_exp={exp}/{roi}_data_nogse_vs_x_tnogse={tnogse}_g={g}_N={int(n)}.txt")
        ...
        model.set_param_hint("D0", value=0.7e-12, min = 0.0, max = D0_ext, vary = 0)
        ...
        fit = nogse.M_nogse_rest(float(tnogse), float(g), n, x_fit, tc_fit, M0_fit, D0_fit)
        ...
        with open(f"../results_{file_name}/{folder}/{roi}_parameters_vs_tnogse_G={num_grad}.txt", "a") as a:
            print(tnogse, g, tc_fit, tc_error, M0_fit, M0_error, D0_fit, D0_error, file=a)
        
        with open(f"../results_{file_name}/{folder}/{roi}_parameters_vs_g_tnogse={tnogse}.txt", "a") as a:
            print(g, tnogse, tc_fit, tc_error, M0_fit, M0_error, D0_fit, D0_error, file=a)
