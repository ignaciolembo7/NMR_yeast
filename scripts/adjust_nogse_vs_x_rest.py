...
modelo = "rest_mode"  # nombre carpeta modelo libre/rest/tort
...
for roi, color in zip(rois,palette):
    ...
    model = lmfit.Model(nogse.M_nogse_rest, independent_vars=["TE", "G", "N", "D0", "x"], param_names=["t_c", "M0"])
    model.set_param_hint("M0", value=1000.0, min = 0.0)
    model.set_param_hint("t_c", value=1.0, min = 0.0)
    params = model.make_params()
    params["M0"].vary = 1
    params["t_c"].vary = 1

    result = model.fit(f, params, TE=float(tnogse), G=float(g), N=n, D0=D0, x=x) # Cambiar el D0 según haga falta puede ser D0_int o D0_ext
    ...
    fit = nogse.M_nogse_rest(float(tnogse), float(g), n, x_fit, t_c_fit, M0_fit, D0)
    ...
    with open(f"../results_{file_name}/{folder}/tauc_vs_tnogse_{num_grad}.txt", "a") as a:
        print(tnogse,t_c_fit,error_t_c,M0_fit,error_M0, file=a)
